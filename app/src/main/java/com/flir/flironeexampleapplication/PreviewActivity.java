...
public class PreviewActivity extends Activity implements Device.Delegate, FrameProcessor.Delegate, Device.StreamDelegate, Device.PowerUpdateDelegate{
    ...
    // FraIme Processor Delegate method, will be called each time a rendered frame is produced
    static Boolean imgIron = false;
    static Boolean imgArtic = false;
    static Boolean imgRainbow = false;
    static Boolean imgColdest = false;
    static Boolean imgContrast = false;
    static Boolean imgGray = false;
    static Boolean imgLava = false;
    static Boolean imgHottest = false;
    static Boolean imgWheel = false;
        ...
        */
        if (this.imageCaptureRequested) {
            ...
            //    public void run() {
                    ...
                    RenderedImage.Palette palette = RenderedImage.Palette.Iron;
                    /*if (!countDown) {
                        try {
                            updateThermalImageView(bitmap1);
                            Thread.sleep(1000);
                            updateThermalImageView(bitmap2);
                            Thread.sleep(1000);
                            updateThermalImageView(bitmap3);
                            Thread.sleep(1000);
                            updateThermalImageView(bitmap4);
                            Thread.sleep(1000);
                            updateThermalImageView(bitmap5);
                        ...
                        path += "/images_" + formatedDate;
                        File folder = new File(path);
                        folder.mkdir();
                        folderPath = folder.toString();

                        try {
                            java.io.FileOutputStream fou = new java.io.FileOutputStream(folderPath + "/ThermalLinearFlux14BitImage.raw");
                            java.io.DataOutputStream outputStreamWriter = new java.io.DataOutputStream(fou);
                            outputStreamWriter.write(renderedImage.pixelData());
                        ...
                        frameProcessor.setImageTypes(EnumSet.of(defaultImageType));
                        return;
                    }else if (renderedImage.imageType() == RenderedImage.ImageType.ThermalRadiometricKelvinImage) {
                        try {
                            java.io.FileOutputStream fou = new java.io.FileOutputStream(folderPath + "/ThermalRadiometricKelvinImage.raw");
                            java.io.DataOutputStream outputStreamWriter = new java.io.DataOutputStream(fou);
                            outputStreamWriter.write(renderedImage.pixelData());
                            outputStreamWriter.close();
                        } catch (IOException e) {
                        ...
                        RenderedImage.ImageType defaultImageType = RenderedImage.ImageType.VisualJPEGImage;
                        //frameProcessor = new FrameProcessor(this, this, EnumSet.of(defaultImageType));
                        frameProcessor.setImageTypes(EnumSet.of(defaultImageType));
                        return;
                    }else if (renderedImage.imageType() == RenderedImage.ImageType.VisualJPEGImage) {
                        try {
                            String fName = folderPath + "/VisualJPEGImage.jpg";
                            java.io.FileOutputStream fou = new java.io.FileOutputStream(fName);
                            java.io.DataOutputStream outputStreamWriter = new java.io.DataOutputStream(fou);
                            //outputStreamWriter.write(renderedImage.pixelData());
                            //outputStreamWriter.close();

                            //Rotar la imagen antes de guardarla
                            Bitmap img = BitmapFactory.decodeByteArray(renderedImage.pixelData() , 0, renderedImage.pixelData().length);

                            //Bitmap img = BitmapFactory.decodeFile(fileName);
                            android.graphics.Matrix  matrix = new android.graphics.Matrix();
                            matrix.postRotate(90);
                            Bitmap imgRot  = Bitmap.createBitmap(img, 0, 0, img.getWidth(), img.getHeight(), matrix, true);
                            ByteArrayOutputStream blob = new ByteArrayOutputStream();
                            imgRot.compress(Bitmap.CompressFormat.JPEG, 100, blob);

                            outputStreamWriter.write(blob.toByteArray());
                            outputStreamWriter.close();
                        ...
                        RenderedImage.ImageType defaultImageType = RenderedImage.ImageType.VisualYCbCr888Image;
                        //frameProcessor = new FrameProcessor(this, this, EnumSet.of(defaultImageType));
                        frameProcessor.setImageTypes(EnumSet.of(defaultImageType));
                        return;
                    }else if (renderedImage.imageType() == RenderedImage.ImageType.VisualYCbCr888Image) {
                        try {
                            String fName = folderPath + "/VisualYCbCr888Image.bmp";
                            File file = new File(fName); // the File to save to
                            java.io.FileOutputStream fOut = new java.io.FileOutputStream(file);
                            thermalBitmap = renderedImage.getBitmap();
                            thermalBitmap.compress(Bitmap.CompressFormat.JPEG,100,fOut);
                            fOut.flush();
                            fOut.close();
                        } catch (IOException e) {
                            Log.e("Exception", "File write failed: " + e.toString());
                        ...
                        palette = RenderedImage.Palette.Iron;
                        fileName = "FLIROne-Iron-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        imgIron = true;
                        ...
                        palette = RenderedImage.Palette.Arctic;
                        fileName = "FLIROne-Arctic-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Coldest;
                        fileName = "FLIROne-Coldest-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Contrast;
                        fileName = "FLIROne-Contrast-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Gray;
                        fileName = "FLIROne-Gray-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Lava;
                        fileName = "FLIROne-Lava-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Hottest;
                        fileName = "FLIROne-Hottest-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Wheel;
                        fileName = "FLIROne-Wheel-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                        ...
                        palette = RenderedImage.Palette.Rainbow;
                        fileName = "FLIROne-Rainbow-" + formatedDate + ".jpg";
                        try {
                            lastSavedPath = folderPath + "/" + fileName;
                            renderedImage.getFrame().save(new File(lastSavedPath), palette, RenderedImage.ImageType.ThermalRGBA8888Image);
                        } catch (Exception e) {
                    ...
                    if (imageCaptureRequested)
                    {
                        try {
                            ...
                            MediaScannerConnection.scanFile(context,
                                    new String[]{folderPath + "/" + fileName}, null,
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                thermalImageView.animate().setDuration(1000).scaleY(0).withEndAction((new Runnable() {
                                    public void run() {
                                        thermalImageView.animate().setDuration(1000).scaleY(1);
                                    }
                                }));
                            }
                        });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bitmap1 = decodeResource(contentView.getResources(), R.mipmap.ic_1);
        bitmap2 = decodeResource(contentView.getResources(), R.mipmap.ic_2);
        bitmap3 = decodeResource(contentView.getResources(), R.mipmap.ic_3);
        bitmap4 = decodeResource(contentView.getResources(), R.mipmap.ic_4);
        bitmap5 = decodeResource(contentView.getResources(), R.mipmap.ic_5);
